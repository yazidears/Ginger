...
export default function ReceptivityMap(props:ReceptivityMapProps){
 const hotspots:GeoJSON.FeatureCollection={type:'FeatureCollection',features:props.data?currentThermals(props.data,props.now).map(h=>({type:'Feature',id:h.id,geometry:{type:'Point',coordinates:h.position},properties:{id:h.id,frp:h.frpMw,source:h.provenance.source,observedAt:h.provenance.observedAt}})):[]};
