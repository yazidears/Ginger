...
export async function fetchJSON<T>(url:string,init:RequestInit={},timeoutMs=6000):Promise<T>{
 ...
 for(let attempt=0;attempt<2;attempt++){
  ...
  try{response=await fetch(url,{...init,signal,cache:'no-store'});}catch(e){
   if(signal.aborted)throw signal.reason;
   if(!retryableMethod||attempt===1)throw Error('Upstream network request failed');
   await pause(150,signal);continue;
  }
  if(!response.ok){const error=new UpstreamHTTPError(response.status);const delay=retryDelay(response.headers.get('retry-after'));await response.body?.cancel();
  ...
  try{return await response.json() as T;}catch{signal.throwIfAborted();throw Error('Upstream returned invalid JSON');}
 }
