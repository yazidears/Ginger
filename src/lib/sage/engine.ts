...
export function simulateLandscape(land:Landscape,request:RunRequest,id:string,onStage:(stage:string)=>void=()=>{},origin=new Date().toISOString(),options:{stencil?:'legacy8'|'ginger16'}={}):RunResult{
  ...
  if(land.weather.some(w=>![w.temperatureC,w.humidityPct,w.windKmh,w.windFromDegrees,w.directNormalWm2,w.diffuseWm2,w.precipitationMm].every(Number.isFinite)||w.humidityPct<0||w.humidityPct>100||w.windKmh<0||w.directNormalWm2<0||w.diffuseWm2<0))throw Error('Invalid weather forcing');
  ...
  const observed=experiment?.observation?localPolygons(experiment.observation.geometry,center):null;
  ...
  const originMs=Date.parse(origin),baseWeather=land.weather.findIndex(w=>Date.parse(w.time)<=originMs&&originMs<Date.parse(w.time)+3600000);
  ...
  const frames=Array.from({length:frameCount},(_,k)=>{
    const t=originMs+k*30*60000,w=land.weather.find(w=>Date.parse(w.time)<=t&&t<Date.parse(w.time)+3600000);
  ...
  const radiation=frames.map(frame=>Array.from({length:total},(_,i)=>cellSolar(i,elevations,surface,size,cellM,frame.sun,frame.weather.directNormalWm2,frame.weather.diffuseWm2)));
  ...
  const results:BuildingResult[]=buildings.map<BuildingResult>(({feature:f,parts,b,height,near,roofCells},buildingIndex)=>{
    ...
    const roofSolar=height===null?[]:roofCells.map(i=>{const shade=shadowed(cellCenter(i,size,cellM),elevations[i]+height+.2,frames[0].sun.altitudeDegrees,frames[0].sun.azimuthDegrees,surface,size,cellM);return {shade,wm2:irradiance(frames[0].weather.directNormalWm2,frames[0].weather.diffuseWm2,frames[0].sun,[0,0],shade)};});
