...
The prevention scheduler runs alongside receptivity both in the web process and the external backend worker. It publishes watch-area evidence before waiting for local GIS plans, then refreshes prepared plans once per minute using the existing provider caches. This schedule uses deterministic proposals without triggering AI inference. Plan evidence and saved-task status load independently; queue failures leave evidence visible and disable task creation until a successful retry.
