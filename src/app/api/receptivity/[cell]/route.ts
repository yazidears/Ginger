...
export async function GET(request:Request,{params}:{params:Promise<{cell:string}>}){
 ...
 if(!station||!assessment)return Response.json({cell:id,available:false,reason:'Required weather or forecast coverage is unavailable.'},{status:200});
 const i=HORIZONS.indexOf(h as Horizon),score=cell.receptivity[i]!;
 return Response.json({cell:id,name:cell.name,center:cell.center,timestamp:assessment.timestamp,computedAt:snapshot.generatedAt,stale:result.stale,horizon:h,fireReceptivity:score,spreadPotential:cell.spread[i],classification:classify(score),confidence:cell.confidence,confidenceMeaning:'Evidence completeness heuristic, not a probability',velocity:cell.velocity,conditions:{...assessment.conditions,...station.history,fineFuelMoisture:assessment.fineFuelMoisture,ffmc:assessment.ffmc,isi:assessment.isi},fuel:cell.fuel,terrain:cell.terrain,provenance:{station:station.station.name,stationDistanceKm:cell.stationDistanceKm,forecastSource:station.forecastSource,forecastIssuedAt:station.forecastIssuedAt,model:snapshot.version},explanation:explainCell(cell,station,h as Horizon)},{headers:{'Cache-Control':'no-store'}});
}
