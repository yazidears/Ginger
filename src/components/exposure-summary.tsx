'use client';
import './exposure-summary.css';
import {exposureCategories, type ExposureSummary} from '@/lib/exposure/types';
export default function ExposureSummaryView({exposure, compact=false, onInspect, context='watch'}: {context?:'watch'|'receptivity'; exposure?: ExposureSummary; compact?: boolean; onInspect?: (lon: number, lat: number, name: string) => void}) {
  ...
  return <details className="signal-evidence exposure-summary" open={compact ? undefined : true}>
    ...
    <p>{exposure.hazardActive ? `Exposure contribution: +${exposure.uplift}/40 priority points.` : context==='receptivity'?'No current high-receptivity trigger. Exposure is recorded for readiness.':'No active weather or heat trigger. Exposure is recorded for readiness.'}</p>
    ...
    <p className="small-source">{context==='receptivity'?'Cell enclosing circle':'Zone'} {exposure.radiusM<1000?`${exposure.radiusM} m`:`${(exposure.radiusM/1000).toFixed(1)} km`} + 1 km nearby buffer · {exposure.status}<br/>OSM source: {exposure.sourceDate?.slice(0,10)} · occupancy unknown</p>
