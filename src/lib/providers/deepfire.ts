...
export function parseHotspots(raw:unknown):Hotspot[]{return collection(raw).features.map(f=>{const p=f.properties||{};if(typeof p.observed_at!=='string'||!Number.isFinite(Date.parse(p.observed_at)))throw Error('Invalid hotspot timestamp');return{id:String(f.id||p.id),position:f.geometry.coordinates.slice(0,2) as [number,number],clusterId:typeof p.cluster_id==='string'?p.cluster_id:null,frpMw:typeof p.fire_radiative_power==='number'?p.fire_radiative_power:null,confidence:String(p.confidence||'UNKNOWN'),provenance:{source:`Deepfire / ${String(p.source||'satellite')}`,observedAt:p.observed_at,retrievedAt:isoNow(),mode:'live'}};});}
